 ...
 */
async function queueNotification({ contactId, checkinId, type, to, subject, body }) {
  const db = getDb();
 ...
 */
async function processNotification(logId) {
  const db = getDb();
  ...
  let result;
  const subject = `Daily Check-in Update for ${row.name}`;
  ...
  if (row.type === 'email' && row.notify_email && row.email) {
    result = await sendEmail({ to: row.email, subject, body, logId });
  } else if (row.type === 'sms' && row.notify_sms && row.phone) {
    result = await sendSMS({ to: row.phone, body, logId });
  } else {
  ...
  // Update log
  const status = result.success ? 'sent' : (row.retry_count < MAX_RETRIES ? 'retrying' : 'failed');
  ...
  if (status === 'retrying') {
    ...
    setTimeout(() => processNotification(logId), RETRY_DELAY);
  }
}
 ...
 */
async function notifyFamily(checkinId, userId) {
  const db = getDb();
  ...
  for (const contact of contacts) {
    if (contact.notify_email && contact.email) {
      ...
      await processNotification(logId);
    }
